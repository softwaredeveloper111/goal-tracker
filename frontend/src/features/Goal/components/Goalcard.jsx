    ...
    <div className=" block group relative bg-[#1a1a1a] p-8 transition-all duration-300 hover:bg-[#1f1f1f]">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        ...
        <div className="flex-1">
          ...
          <Link
            to={`/goals/${goal._id}`}
            className="hover:text-amber-400 duration-700 block text-2xl font-black text-white font-['Space_Grotesk'] mb-3"
          >
            {goal.title}
          </Link>
        ...
        <div className="flex flex-col items-end">
          <div className="flex items-center gap-2 bg-[#0e0e0e] px-4 py-2 border border-[#00ff87]/10">
            ...
            <span className="text-2xl font-black text-[#00ff87] font-['Space_Grotesk']">
              ...
              <GoalProgressBar
                checkedDays={goal.checkedDays ?? 0}
                totalDays={totalDays}
              />
            </span>
          </div>
      ...
      <div className="absolute  top-1 right-1">
        <DeleteGoalButton onConfirm={() => HandleDeleteGoalAPI(goal._id)} />
      </div>
    </div>
  );
}
