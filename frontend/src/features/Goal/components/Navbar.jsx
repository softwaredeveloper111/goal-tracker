...
export default function Navbar() {
  ...
  useEffect(() => {
   ...
   async function LogoutHandler(){
    try {
     const response =  await HandlerLogoutAPI();
     if(response){
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 h-16 bg-[#0e0e0e]/70 backdrop-blur-xl border-b border-[#1a1a1a] font-['Space_Grotesk']">
      ...
      <Link
        to="/"
        className="text-xl font-black tracking-[0.15em] text-[#00ff87] uppercase flex items-center gap-2"
      >
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="absolute right-0 top-12 w-48 bg-[#111] border border-[#1a1a1a] shadow-[0_8px_30px_rgba(0,0,0,0.5)] z-50">
            <label className="cursor-pointer w-full flex items-center gap-3 px-4 py-3 text-[#555] hover:text-white hover:bg-[#1a1a1a] transition-all duration-200 text-[11px] uppercase tracking-widest font-['Space_Grotesk']">
              <input
                type="file"
                accept="image/*"
                className="hidden"
